...
void main() {
  ...
  group('AyahTimingService', () {
    test(
      'getChapterTimings returns empty list if both asset and API fail',
      () async {
        // Arrange
        const reciterId = 999;
        const chapterNumber = 1;
        when(
          () => mockDataSource.fetchChapterTiming(reciterId, chapterNumber),
    ...
    test(
      'getChapterTimings falls back to API and caches result dynamically when asset is missing',
      () async {
        // Arrange
        const reciterId = 888;
        ...
        final timings1 = await timingService.getChapterTimings(
          reciterId,
          chapterNumber,
        );
        ...
        final timings2 = await timingService.getChapterTimings(
          reciterId,
          chapterNumber,
        );
    ...
    test('getAyahTiming returns null when fetching timing fails', () async {
      // Arrange
      when(
        () => mockDataSource.fetchChapterTiming(any(), any()),
      ).thenAnswer((_) async => null);
    ...
    test(
      'getAyahTiming returns specific verse from dynamically cached timings',
      () async {
        // Arrange
        const reciterId = 777;
        ...
        // Act
        final timing = await timingService.getAyahTiming(
          reciterId,
          chapterNumber,
          2,
        );
    ...
    test(
      'getCurrentVerse returns correct verse based on playback time',
      () async {
        // Arrange
        const reciterId = 666;
        const chapterNumber = 1;
        ...
        // Act
        final verseNull = await timingService.getCurrentVerse(
          reciterId,
          chapterNumber,
          -50,
        );
        final verse1 = await timingService.getCurrentVerse(
          reciterId,
          chapterNumber,
          2500,
        );
        final verse2 = await timingService.getCurrentVerse(
          reciterId,
          chapterNumber,
          5000,
        );
        final verseEnd = await timingService.getCurrentVerse(
          reciterId,
          chapterNumber,
          15000,
        );
    ...
    test(
      ...
      () async {
        ...
        expect(
          timings,
          isEmpty,
          reason: 'Should catch exception and return empty list',
        );
        verify(
          () => mockDataSource.fetchChapterTiming(reciterId, chapterNumber),
        ).called(1);
    ...
    test(
      'getChapterTimings returns empty list if no asset exists and no dataSource is provided',
      () async {
        // Arrange
    ...
    test('getAyahTiming returns null for non-existent verse', () async {
      ...
      const reciterId = 444;
      const chapterNumber = 114;
      final mockApiResult = [AyahTiming(ayah: 1, startTime: 0, endTime: 2000)];
      when(
        () => mockDataSource.fetchChapterTiming(reciterId, chapterNumber),
      ).thenAnswer((_) async => mockApiResult);
